Start-Transcript -Path "C:\Logs\Remove-BloatwarePackages_$(((Get-Date).ToUniversalTime()).ToString("yyyyMMdd_hhmmZ")).log" -Force
...
$officeUninstallStrings = (Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object { $_.DisplayName -like "*Microsoft 365*" -or $_.DisplayName -like "*Office*" } | Select-Object UninstallString).UninstallString
...
if (($results.Count) -eq 0) {
    ...
    foreach ($app in $results) {
        ...
        try {
            # Get-AppxPackage -AllUsers -Name $app -ErrorAction Continue | Remove-AppxPackage -ErrorAction Continue | Out-Null
            winget remove $app
        } catch {
